...
class CheckoutView(LoginRequiredMixin, generic.FormView):
    def get(self, *args, **kwargs):
        ...
        try:
            ...
            if order.produk_items.count() == 0:
                messages.warning(self.request, "Your order doesn't exist yet, continue shopping")
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            messages.warning(self.request, 'Failed to make payment')
    ...
    if request.user.is_authenticated:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                ...
                pesan = f"ProductItem has been updated to: { order_produk_item.quantity }"
                messages.info(request, pesan)
                return redirect('toko:produk-detail', slug = slug)
            else:
                order.produk_items.add(order_produk_item)
                messages.info(request, 'The ProductItem you selected has been added')
                return redirect('toko:produk-detail', slug = slug)
        else:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                try: 
                    ...
                    pesan = f"ProductItem has been deleted"
                    messages.info(request, pesan)
                    return redirect('toko:produk-detail',slug = slug)
                except ObjectDoesNotExist:
                    print('Error: ProductItem orders are no longer available')
            else:
                messages.info(request, 'ProductItem not available')
                return redirect('toko:produk-detail',slug = slug)
        else:
...
def paypal_return(request):
    if request.user.is_authenticated:
        try:
            ...
            messages.info(request, 'Payment has been received, thank you')
            return redirect('toko:home-produk-list')
        except ObjectDoesNotExist:
            messages.error(request, 'Double check your order')
            return redirect('toko:order-summary')
    else:
        return redirect('/accounts/login')
